			...
			<tr>
				...
				<td width="100%">
					<table border="0" width="95%" align="center" margin="0px">
						<tr>
							<td colspan="2">
								<button type="button" id="bookpage" onclick="movebook();" style="background-color:#b5dbf0;"><span style="font-size: 22px;">북마크확인</span></button>
						...
						<tr id="bnoval" class="bnoval" onclick="view('<%= link_str %>');" style="cursor: pointer;">
							...
							<%
							int up_total =  dto.recoTotal(vo.getBno());
							%>
							<%= up_total %>
							</td>
							<td width="30px">
				...
				<table>
					<tr>
						...
						for(int pno = startBlock ; pno <= endBlock; pno++)
						{
							...
							{
								//현재 페이지 이면....
								%><a href="bookmark.jsp?page=<%= pno %>&uno=<%= login.getUno() %>&kind=<%= kind%>"><b style="color:red;">&nbsp;&nbsp;&nbsp;<%= pno %></b></a>&nbsp;<%
							}else
							{
								%><a href="bookmark.jsp?page=<%= pno %>&uno=<%= login.getUno() %>&kind=<%= kind%>">&nbsp;&nbsp;&nbsp;<%= pno %></a>&nbsp;<%	
							}
						}
