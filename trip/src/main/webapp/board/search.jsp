	...
	<table border="1" style="width: 90%; font-size: 18px;" align="center">
		...
		for(boardVO vo : list)
		{
			...
			<tr onclick="view('<%= link_str %>');" style="cursor: pointer;">
				<td><%= seqNo-- %></td>
				<td>
					<span style="width: 130px; font-size: 13px;">
						<span style="color: hotpink; width: 20px; margin:1px;">
							<%= vo.getSeason() %>
						</span>
						<span style="color: saddlebrown; width: 20px; ">
							<%= vo.getLocal() %>
						</span>
						<span style="color: orangered; width: 20px; margin:1px;">
							<%=(vo.getHuman() != null && !vo.getHuman().equals("null"))?vo.getHuman():"" %>
						</span>
						<span style="color: royalblue; width: 20px; margin:1px;">
							<%=(vo.getMove()!= null && !vo.getMove().equals("null"))?vo.getMove():"" %>
						</span>
						<span style="color: darkmagenta; width: 20px; margin:1px;">
							<%=(vo.getSchedule()!= null &&!vo.getSchedule().equals("null"))?vo.getSchedule():"" %>
						</span>
						<span style="color: #FFBF00; text-shadow: 0.5px -1px 0.5px black; width: 20px; margin:1px;">
							<%=(vo.getUinout()!= null &&!vo.getUinout().equals("null"))?vo.getUinout():"" %>
						</span>
					</span>
				</td>
				<td>
		...
		<tr style="font-size: 25px;">
			...
			{
				%>
				<a href="search.jsp?<%= param %>&page=1">◁◁</a>&nbsp;
			...
			if(endBlock < maxpage)
			{
				...
				<a href="search.jsp?<%= param %>&page=<%= maxpage %>">▷▷</a>&nbsp;
				<%
			}
			%>
			</td>
